...
const OUTPUT_PATH = path.join(__dirname, '..', '..', 'HACCP_Orale_v1.4_EN.html');
...
const SECCION_0 = `
<h2 id="sec-0-1">0.1 What HACCP Means</h2>
<p><strong>HACCP</strong> stands for <em>Hazard Analysis and
Critical Control Point</em>. It is a system that identifies,
evaluates and controls the hazards that could make a food unsafe
to eat.</p>
<p>A <strong>hazard</strong> is any biological (bacteria, viruses,
parasites), chemical (cleaning residues, undeclared allergens) or
physical (glass, metal, bone) agent capable of causing harm to
the person eating the food.</p>
<p>HACCP is a <strong>preventive</strong> system: rather than
waiting for the finished product and checking whether it turned
out fine, it controls the process at every stage so the hazard
never has the chance to materialise. Action is taken before the
problem occurs, not after.</p>

<h2 id="sec-0-2">0.2 Prerequisites and HACCP</h2>
<p><strong>Prerequisites</strong> are the basic hygiene conditions
that must exist before and throughout the system: adequate
premises, equipment in good condition, cleaning and disinfection,
pest control, personal hygiene, staff training, supplier control
and waste management.</p>
<p>HACCP is built on top of the prerequisites — it does not
replace them. Without solid prerequisites, the HACCP system does
not hold: there is little point monitoring a critical control
point if the kitchen is not clean or staff do not wash their
hands.</p>
<p>In this plan, prerequisites are developed in <strong>Section
10 (Prerequisite Programmes)</strong> and critical control points
in <strong>Section 6 (Critical Control Points)</strong>.</p>

<h2 id="sec-0-3">0.3 The Seven Codex Alimentarius Principles</h2>
<p>HACCP is organised around seven principles set out by the
Codex Alimentarius. This plan applies all seven; each one is
developed in a specific section of this document:</p>
<ol>
  <li><strong>Conduct a hazard analysis.</strong> Identify what
  could go wrong at each stage of the process. Developed in
  <strong>Section 5 (Hazard Analysis)</strong>.</li>
  <li><strong>Determine the critical control points (CCPs).</strong>
  Identify at which stages controlling a hazard is essential.
  Developed in <strong>Section 6.1 (Decision Tree)</strong>.</li>
  <li><strong>Establish critical limits.</strong> Set the
  measurable value (for example, a temperature) that separates
  safe from unsafe at each CCP. Developed in <strong>Section
  6</strong>, within each CCP record.</li>
  <li><strong>Establish a monitoring system.</strong> Define what
  is measured, how, when and by whom. Developed in <strong>Section
  7.1 (Monitoring Summary)</strong>.</li>
  <li><strong>Establish corrective actions.</strong> Define what
  to do when a deviation from a critical limit is detected.
  Developed in <strong>Section 7.3 (Corrective Action
  Protocol)</strong>.</li>
  <li><strong>Establish verification procedures.</strong> Confirm
  that the system works as designed. Developed in <strong>Section
  8 (Verification and Review)</strong>.</li>
  <li><strong>Establish documentation and record-keeping.</strong>
  Keep written evidence that controls were applied. Developed in
  <strong>Section 9 (Documentation and Records)</strong>.</li>
</ol>

<h2 id="sec-0-4">0.4 What Is a Critical Control Point</h2>
<p>A <strong>critical control point (CCP)</strong> is a stage of
the process where an essential control can be applied to prevent,
eliminate or reduce a hazard to an acceptable level.</p>
<p>The difference from a prerequisite is that a CCP has a
<strong>measurable critical limit</strong> (for example, ≥75°C),
a <strong>defined monitoring procedure</strong> (what, how, when
and by whom) and a <strong>documented corrective action</strong>
if the limit is not met. A prerequisite, by contrast, is a
general hygiene condition without that level of point-specific
control.</p>
<p>The CCPs in this plan were determined by applying the Codex
Alimentarius decision tree to each stage of the process — see
<strong>Section 6.1 (Decision Tree)</strong>.</p>

<h2 id="sec-0-5">0.5 Órale's Classification</h2>
<p>Órale is classified as a <strong>HIGH-RISK operation</strong>
<span class="badge-riesgo">HIGH-RISK</span>, under the FSAI
Guidance Note No. 16 Food Stalls (Revision 2), for preparing,
cooking and directly selling hot food that requires temperature
control.</p>
<p>This classification carries additional requirements for
equipment, personal hygiene, temperature control and
record-keeping: the general requirements of GN-16 §4.3 and the
specific requirements for high-risk activities of GN-16 §5.1.</p>
<p>Órale's full classification is declared in <strong>Section 1.4
(Operation Classification)</strong>.</p>

<h2 id="sec-0-6">0.6 Legal Framework</h2>
<p>This plan responds to the following legal obligations:</p>
<ul>
  <li><strong>Regulation (EC) No. 852/2004, Article 5:</strong>
  requires food business operators to implement permanent
  procedures based on HACCP principles.</li>
  <li><strong>Transposition into Irish law:</strong> European
  Communities (Hygiene of Foodstuffs) Regulations 2006, S.I. No.
  369 of 2006.</li>
  <li><strong>FSAI Guidance Note No. 16 Food Stalls (Revision
  2):</strong> specific guidance for food stalls, used as the
  primary regulatory reference of this plan.</li>
  <li><strong>Regulation (EC) No. 178/2002:</strong> sets out the
  general principles of food law, including traceability and the
  withdrawal of unsafe products.</li>
</ul>
<p>The full regulatory framework applicable to Órale is developed
in <strong>Section 1.3 (Applicable Regulatory Framework)</strong>.</p>

<h2 id="sec-0-7">0.7 How to Use This Plan</h2>
<p>This plan is meant to be consulted, not just filed away. A
quick orientation:</p>
<ul>
  <li><strong>Daily procedures</strong> for hygiene, cleaning and
  food handling are in <strong>Section 10 (Prerequisite
  Programmes)</strong>, with the specific control points and
  their limits in <strong>Section 6 (Critical Control
  Points)</strong>.</li>
  <li>The <strong>recording formats</strong> to be completed each
  working day are listed in <strong>Section 9 (Documentation and
  Records)</strong> and available in full in <strong>Section 13
  (HACCP Recording Formats)</strong>.</li>
  <li>When a <strong>deviation</strong> from a critical limit
  occurs (for example, a temperature out of range), follow the
  <strong>Corrective Action Protocol in Section 7.3</strong>.</li>
</ul>
`;
...
<table>
  ...
  <tbody>
    <tr>
      <td>1</td>
      <td>In-person purchase of raw materials</td>
      <td>
        <ul>
          <li>In-person purchase at regular stores (Musgrave
          MarketPlace, Limerick Fruit Suppliers, mexicanthings,
          Dunnes, Tesco, Lidl). No deliveries are received from
          suppliers</li>
          <li>Selection criteria at the store: see §10.8</li>
          <li>Chilled product purchased at the end of the
          shopping route; transported in an insulated bag with
          ice packs (§10.8, GN-16 §4.3.11)</li>
          <li>Purchase recorded in HACCP-04</li>
        </ul>
      </td>
    </tr>
    <tr class="alt">
      <td>2</td>
      <td>Transport and storage in the domestic kitchen</td>
      <td>
        <ul>
          <li>Temperature checked on arrival with a probe
          thermometer (§10.8); discard chilled product above 5°C</li>
...
<table class="tabla-ancha">
  ...
  <tbody>
    <tr class="celda-riesgo-alto">
      <td>Purchase and transport of raw materials</td>
      ...
      <td>
        <ul>
          <li>Verify temperature ≤5°C on arrival</li>
          <li>Selection at the store (§10.8)</li>
          <li>Visual inspection</li>
          <li>Prerequisite: in-person purchase, transport in an
          insulated bag and traceability (§4.1, §10.8, HACCP-04)</li>
        </ul>
      </td>
      <td>NO</td>
    </tr>
...
<table>
  ...
  <tbody>
    ...
    <tr class="alt">
      <td>HACCP-04</td>
      <td>Purchase control and supplier traceability</td>
      <td>Every purchase</td>
      <td>Domestic kitchen</td>
    </tr>
    <tr>
      <td>HACCP-05</td>
...
<h2 id="sec-10-8">10.8 Purchasing and Transport from the Supplier</h2>
<p>Órale purchases all of its ingredients in person: the team
goes to the store, selects the product and transports it to the
production kitchen. No deliveries are received from suppliers.
No frozen product is purchased either: freezing takes place in
the kitchen after production (CCP4).</p>
<p><strong>Transport requirements (GN-16 §4.3.11):</strong></p>
<ul>
  <li>Chilled product is transported in an insulated bag with
  ice packs, keeping it at ≤5°C throughout the journey.</li>
  <li>Ice packs are kept in the freezer and placed in the bag
  before leaving to shop.</li>
  <li>Chilled product is purchased at the end of the shopping
  route, to minimise time outside refrigeration.</li>
  <li>Raw food of animal origin is transported separately from
  ready-to-eat product and from vegetables.</li>
  <li>On arrival at the kitchen, the product is stored
  immediately under refrigeration and its temperature is
  checked with a disinfected probe thermometer.</li>
  <li>If chilled product is above 5°C on arrival, it is
  discarded. Record in HACCP-07.</li>
</ul>
<p><strong>Selection criteria at the store:</strong></p>
<ul>
  <li>Check the best-before or use-by date; do not buy product
  close to expiry unless it will be used the following day.</li>
  <li>Check packaging integrity: no tears, no dents in cans, no
  swollen packaging.</li>
  <li>Check the appearance, colour and smell of fresh product.</li>
  <li>Check that chilled product is displayed in a chilled
  counter; do not buy product displayed outside its chilled
  equipment.</li>
...
<table>
  ...
  <tbody>
    ...
    <tr class="alt">
      ...
      <td>
        <ul>
          <li>Órale buys in person, with no supplier deliveries
          (§10.8). Keep a record of: store, product type,
          purchase date</li>
          ...
          traceability record under GN-16 §3.3.4. The spreadsheet
          records the amount by store; the detail of products
          purchased is captured in HACCP-04 and on the retained
          receipt. HACCP-04 links each product to its receipt
          reference, so that in the event of an incident the
          source of the affected ingredient can be identified.</li>
...
<h3 id="haccp-04">HACCP-04 — Purchase Control and Supplier
Traceability</h3>
...
const ANEXO = `
<h2>Purchasing — check before going shopping</h2>
<ul class="checklist">
  <li><label class="checklist-item">
    <input type="checkbox" class="checklist-check">
    <span>Insulated bag for transporting purchases</span>
  </label></li>
  <li><label class="checklist-item">
    <input type="checkbox" class="checklist-check">
    <span>Frozen ice packs, ready before leaving</span>
  </label></li>
  <li><label class="checklist-item">
    <input type="checkbox" class="checklist-check">
    <span>Probe thermometer to check on arrival</span>
  </label></li>
  <li><label class="checklist-item">
    <input type="checkbox" class="checklist-check">
...
<table class="tabla-ancha">
  ...
  <tbody>
    ...
    <tr class="alt">
      <td>HACCP-04</td>
      <td>Purchase and traceability control</td>
      <td>—</td>
      <td>Each purchase</td>
      <td>Domestic kitchen</td>
